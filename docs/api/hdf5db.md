...
This module defines `Hdf5db`, the in-memory object store that sits between the h5json library's storage backends (`h5reader`/`h5writer` implementations) and everything else that manipulates HDF5 metadata and data. `Hdf5db` keeps a dictionary of object JSON (`_db`, keyed by object id) plus sets tracking which objects are new, dirty, resized, or deleted since the last flush, and a per-dataset list of pending, not-yet-flushed value updates. Reads (`getObjectById`, `getDatasetValues`, etc.) transparently overlay these in-memory changes on top of whatever the reader last supplied, and `flush()`/`close()` push everything through the writer. The module also defines the `ChunkIterator` helper class used to walk a dataset's values chunk by chunk without loading the whole dataset into memory.
...
Property. Returns the current `H5Reader` instance, or `None` if none is set. The setter flushes the writer (if set and open) and closes the current reader (if open) before installing the new reader and calling `set_db()` on it so it can access this `Hdf5db` instance.

### Hdf5db.writer

Property. Returns the current `H5Writer` instance, or `None` if none is set. The setter closes the current writer (if any) before installing the new one and calling `set_db()` on it.
...
Writes out all pending changes (new, dirty, deleted, and resized objects, plus queued dataset value updates) via the writer. Calls `_checkWriter()` first, which raises `IOError` if no writer is set or the writer is closed. Returns `False` without clearing any tracking sets if `writer.flush()` reports failure; otherwise clears the new/dirty/deleted/resized sets and the pending dataset-updates map, and returns `True`.
...
Opens the reader and writer (installing `H5NullReader`/`H5NullWriter` if either was not set). Raises `IOError` if the reader is already open. If the writer is in append mode, the writer is opened first to obtain the root id, then the reader is opened and its root id checked for consistency; otherwise the reader is opened first and the writer opened second. Raises `IOError` on any root-id mismatch between reader, writer, and any previously-set `root_id`. Returns the root object id.

### Hdf5db.close()

Flushes and closes the writer (unless it is a no-op `H5NullWriter`), then closes the reader if one is set.

### Hdf5db.closed

Property. Returns `True` if the reader (or, failing that, the writer) reports closed, or if no reader/writer is set but a `root_id` has been established; returns `False` if no reader, writer, or root id is present.

### Hdf5db.getObjectById(obj_id, refresh=False)

Returns the JSON representation of the object identified by `obj_id`. Calls `_checkReader()` first (raises `IOError` if no reader is set or the reader is closed). If the object is not already cached in the db, or `refresh=True` and the object is neither new nor dirty, fetches it fresh from the reader and stores it in the db. Returns the (possibly in-memory-modified) object JSON, or `None` if the object was deleted.
...
Evaluates the boolean expression `query` against dataset `dset_id`, restricted to selection `sel` (defaults to the whole dataset), and returns a numpy array of coordinate indices (shape `(N, rank)`) for the matching elements, capped at `limit` elements if `limit > 0`. Delegates to `reader.queryDataset()` when the reader can answer for the requested region and no pending update supersedes it, falling back to iterating the selection chunk by chunk (via `ChunkIterator`) and evaluating `arrayQuery` on each chunk, or on the whole selection at once if a `ChunkIterator` cannot be constructed for it (e.g. point/fancy selections). Pending in-memory updates are queried directly and merged into (or subtracted from, where they invalidate stale reader results) the match mask. If `update_value` is provided, the matching elements (up to `limit`) are overwritten with `update_value` (which may be a dict mapping field names to values for a compound dtype, updating only those fields) by first flushing the db and delegating to `writer.queryDataset()`/`setDatasetValues()`. Raises `TypeError` for a non-string `query`, non-`Selection` `sel`, or negative `limit`, and `ValueError` for null-space datasets.
