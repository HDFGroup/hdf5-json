...
def doAttributeCreate(attr_json, parent_id):
    ...
    print(
        indent + "call CreateAttribute('" + attr_name + "', " + parent_id + ", "
...
def doLink(h5json, link_json, parent_id, h5path):
    ...
    if link_json["class"] == "H5L_TYPE_EXTERNAL":
        print("{0}call h5lcreate_external_f('{1}', '{2}', {3}, '{4}', i_res)".format(
            indent, link_json["title"], link_json["h5path"], parent_id, link_json["title"]))
    elif link_json["class"] == "H5L_TYPE_SOFT":
        print("{0}call h5lcreate_soft_f('{1}', {2}, '{3}', i_res)".format(
            indent, link_json["h5path"], parent_id, link_json["title"]))
        ...
        if link_json["collection"] == "groups":
            ...
            print("{0}call {1}('{2}', {3}, i_res)".format(
                indent, subroutine_name, link_json['title'], parent_id))
